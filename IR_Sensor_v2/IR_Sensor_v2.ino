/********************************************************************************
 *
 * $Workfile: IR_Sensor_v2.ino $
 *
 * $Creator: Jonathan Vargas $
 *
 * $Description: PID Control speed of BLDC motor using IR sensosr. $
...
volatile int how_many_rotations = 10; // After How many rotations it prints "Home position"?
volatile int tempo_rotations = 0;    
...
void setup() {
  ...
  Serial.println("===============================================================");
  Serial.println("	SPEED CONTROLLER USING IR SENSOR ");
...
{  
  if(count2!=0){
    ...
    cli();
    tempo_rotations++;   
    if(tempo_rotations>=how_many_rotations){
